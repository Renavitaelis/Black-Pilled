...
/* Private user code ---------------------------------------------------------*/
/* USER CODE BEGIN 0 */
 uint16_t MemEstadoSwitch = 2; //Co-Variavel para atualização do LCD
...
{
	if(Estado != MemEstadoSwitch) //Houve mudança de estado?
		if (Estado) // 1
		{
		  LCD_Clear_All(LCD);
		  LCD_New_XY(LCD, 0, 0);
		  LCD_Put_String(LCD, "PreScaler: ");
		  LCD_Put_Converted_StringU16(LCD, Timer->Instance->PSC);

		  LCD_New_XY(LCD, 0, 1);
		  LCD_Put_String(LCD, "PeriodoMS: ");
		  LCD_Put_Converted_StringF(LCD, PeriodoMS);
		}
		else //0
		{
		  LCD_Clear_All(LCD);
		  ...
		  Timer->State==HAL_TIM_STATE_RESET?LCD_Put_String(LCD, "Inativo"):LCD_Put_String(LCD, "Ativo");

		  LCD_New_XY(LCD, 0, 1);
		  LCD_Put_String(LCD, "Contando:");
		  LCD_Put_Converted_StringU16(LCD, ( ( (Timer->Instance->PSC * Timer->Instance->CNT) / 12000) * 100 / PeriodoMS ) );
		  LCD_Put_Char(LCD,'%');
		}
	else //Então houve atualização de números
		if (Estado) //1
		{
			LCD_Clear_Partial_Row(LCD, 0, 11, 16);
			LCD_New_XY(LCD, 11, 0);
			LCD_Put_Converted_StringU16(LCD, Timer->Instance->PSC);

			LCD_Clear_Partial_Row(LCD, 1, 11, 16);
			LCD_New_XY(LCD, 11, 1);
			LCD_Put_Converted_StringF(LCD, PeriodoMS);
		}
		else // 0
		{
			LCD_Clear_Partial_Row(LCD, 1, 9, 16);
			LCD_New_XY(LCD, 9, 1);
			LCD_Put_Converted_StringU16(LCD, ( ( (Timer->Instance->PSC * Timer->Instance->CNT) / 12000) * 100 / PeriodoMS ) );
			LCD_Put_Char(LCD,'%');
		}

	MemEstadoSwitch = Estado;
}

void AtualizaPSCTimer(TIM_HandleTypeDef * Timer, uint16_t NovoPSC)
{
	Timer->Instance->PSC = NovoPSC;
	PeriodoMS = ( (NovoPSC * Timer->Instance->ARR ) / 12000);
	Timer->Instance->CNT = Timer->Instance->ARR;

	AtualizaLCD(&lcd1, Timer, EstadoSwitch);
}
 /* USER CODE END 1 */
/**
	...
	for(int i=0;i<8/*8*10ms = 80 ms*/;i++)
		{
			...
			if(HAL_GPIO_ReadPin(GPIOB, Bot_Red_Pin))
			{
				...
				PSCMod==20?
					PSCMod=65520 //Vai ao valor máximo, Período de 5min + 50.53s (350.53s)
			...
			if(HAL_GPIO_ReadPin(GPIOB, Bot_Aum_Pin))
			{
				...
				PSCMod==65520?//Limite uint16_t == 65535 | 65520/20 == 3276 | 3276 * 107 == 350 532 ms == 350.53s
