...
int main(void)
{
  ...
  while (1)
  {
    ...
    for (int i = 0; i < 5; i++)
    {
      ...
      // Conta de 0 a 31 nos LEDs:
      HAL_GPIO_WritePin(GPIOA, conta_led<<3, 1);
      HAL_GPIO_WritePin(GPIOA, ~(conta_led)<<3, 0);      
      conta_led++;
      HAL_Delay(500); // Para ver o efeito da contagem (até 31 antes de resetar)
    }

    HAL_Delay(81); // Queremos que a luz do black pisque a cada 405ms em parelelo, então (5*81ms) para diminuir delay
